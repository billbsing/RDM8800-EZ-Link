...
/* Changed by Bill for access to Type B cards 							*/
/* especially the Singapore EZ-Link card.								*/
...
/*Construct PN532Lib object and construct PN532's own SPI interface object */
/* at the same time.*/
PN532Lib::PN532Lib(uint8_t cs):
pn532_SPI()
{
	_cs = cs;
	pinMode(_cs,OUTPUT);
...
void PN532Lib::begin() 
{
	...
	pn532_SPI.setClockDivider(SPI_CLOCK_DIV8);
...
uint32_t PN532Lib::getFirmwareVersion(void) 
{
	uint32_t response;
...
boolean PN532Lib::sendCommandCheckAck(uint8_t* cmd, uint8_t cmd_len, uint16_t timeout) 
{
	uint16_t timer = 0;
...
boolean PN532Lib::SAMConfig(void) 
{
...
uint32_t PN532Lib::configurePeerAsInitiator(uint8_t baudrate) 
{
...
/**********************************************************************/
/*Function: Perform a data exchange between the connected card	 		*/
/*		Parameters:
/*			dataOut: sends out the data to the card						*/
/*			dataOutLength: length of the data to send out 				*/
/*			dataIn:	buffer to put the received data into				*/
/*			dataInLength: length of data to receive and the size 		*/
/*				of the allocated memery in dataIn buffer 				*/
/*																		*/
/*Return: boolean,true if the data was successfully transfered.		 	*/
boolean PN532Lib::dataExchange(uint8_t *dataOut, uint16_t dataOutLength, uint8_t *dataIn, uint16_t dataInLength)
{
	uint16_t i;
	// add the 'In Data Exchange' packet command
	pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
	
	//check that the outData length is not bigger than the packetBuffer length.
	if (dataOutLength > PN532_PACK_BUFF_SIZE ) {
		dataOutLength = PN532_PACK_BUFF_SIZE;
	}
	// copy over the data to the packet buffer
	for ( i = 0; i < dataOutLength; i ++ ) {
		pn532_packetbuffer[i + 1] = dataOut[i];
	}
	
	// send out the packet data to the card
	if (! sendCommandCheckAck(pn532_packetbuffer, dataOutLength + 1))
		return false;

	// clear out the dataIn buffer
	memset(dataIn, 0, dataInLength);
	
	// read all data into the dataIn buffer
	read(dataIn, dataInLength);
	...
	// now check the response to see if the correct values came back
	// response command == 0x41 and status == 0x00
	if ( dataIn[6] == 0x41 && dataIn[7] == 0x00 ) {
		// if so move the actual recevied data to the begining of the buffer
		memmove(&dataIn[0], &dataIn[8], dataInLength - 8);
		return true;
	}
	return false;
...
boolean PN532Lib::initiatorTxRx(char* dataOut,char* dataIn)
{
	pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
...
uint32_t PN532Lib::configurePeerAsTarget() 
{
	byte pbuffer[38] =      { 
...
uint32_t PN532Lib::targetTxRx(char* dataOut,char* dataIn)
{
...
uint32_t PN532Lib::authenticateBlock(uint8_t cardnumber /*1 or 2*/,uint32_t cid /*Card NUID*/, uint8_t blockaddress /*0 to 63*/,uint8_t authtype/*Either KEY_A or KEY_B */, uint8_t * keys) 
{
	pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
...
/*Return:   boolean         					      	    */
boolean PN532Lib::readMemoryBlock(uint8_t cardnumber,uint8_t blockaddress,uint8_t * block)
{
	pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
	pn532_packetbuffer[1] = cardnumber;  // either card 1 or 2 (tested for card 1)
...
boolean PN532Lib::writeMemoryBlock(uint8_t cardnumber,uint8_t blockaddress,uint8_t * block) 
{
	pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
	pn532_packetbuffer[1] = cardnumber;  // either card 1 or 2 (tested for card 1)
...
/****************************************************************************/
/*Function: Read a passive target Id.		                            	*/
/*Parameter:-uint8_t cardBaudrate;                             				*/
/*	    	-uint8_t card type, Type A or B							.       */
/*Return:  tag Id														    */
/*																			*/
uint32_t PN532Lib::readPassiveTargetID(uint8_t cardbaudrate, uint8_t cardType) 
{
	...
	if ( cardType == PN532_CARDTYPEB ) {
		packetLength = 4;
	}

	if (! sendCommandCheckAck(pn532_packetbuffer, packetLength))
	...
	// read data packet
	read(pn532_packetbuffer, packetLength);
	// check some basic stuff
	...
	ID_LENGTH = pn532_packetbuffer[12];
	cid = 0;
...
uint8_t PN532Lib::readSpiStatus(void) 
{
	digitalWrite(_cs, LOW);
	delay(2);
...
/*Return:  void                                                       */
void PN532Lib::read(uint8_t* buff, uint8_t n) 
{
	digitalWrite(_cs, LOW);
	delay(2);
...
void PN532Lib::writeCommand(uint8_t* cmd, uint8_t cmd_len) 
{
	uint8_t checksum;
