...
#define PN532_CARDTYPEA			0x1
#define PN532_CARDTYPEB			0x2
...
class PN532Lib
{
	public:
		...
		uint32_t readPassiveTargetID(uint8_t cardbaudrate, uint8_t cardType = PN532_CARDTYPEA);
		...
		boolean sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout = 500);
		boolean dataExchange(uint8_t *dataOut, uint16_t dataOutLength, uint8_t *dataIn, uint16_t dataInLength);
